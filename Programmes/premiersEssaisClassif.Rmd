...
# Package fpca
On sélectionne un échantillon de courbes journalières. On les visualise.
```{r}
load("Donnees/df.constr.table.RData")
df.constr.table$station.jour <- paste(df.constr.table$number, df.constr.table$download_date_trunc)
ech=sample(unique(df.constr.table$station.jour), 500)
df.constr.table.ech <- df.constr.table %>%
  filter(station.jour %in% ech & download_hour %in% c(0, 3, 6, 9, 12, 15, 18, 21))
# print(ggplot(df.constr.table.ech) + aes(x=download_date, y=taux_dispo) + geom_line(col="blue") + ylim(c(0,1)) + facet_wrap(~ number + download_date_trunc, scales="free") + theme_bw() + theme(axis.text.x=element_blank(), axis.text.y=element_blank()))
```


```{r}
df.fpca<- df.constr.table.ech[, c("number", "taux_dispo", "download_date")]
df.fpca$download_date <- as.numeric(df.fpca$download_date, origin="1970-01-01")
min.date <- min(df.fpca$download_date)
max.date <- max(df.fpca$download_date)
df.fpca$download_date <- (df.fpca$download_date - min.date) / (max.date - min.date)
...
result<-fpca.mle(df.fpca, M.set,r.set,ini.method, basis.method,sl.v,max.step,grid.l,grids)
